        ...
        <div className="module-content visual-content">
          ...
          <section className={`visual-handoff${handoffReady ? '' : ' blocked'}`}>
            ...
            {scriptRecord ? <>
              ...
              <div className="visual-handoff-actions"><button type="button" aria-expanded={scriptOpen} onClick={() => setScriptOpen((open) => !open)}>{scriptOpen ? 'Hide full Script' : 'View full Script'}</button><Link href="/studio/voiceover" onClick={(e) => studioNavigate('/studio/voiceover', e)}>Back to Voiceover</Link></div>
              {scriptOpen ? <div className="visual-source-full"><ScriptDocumentView content={scriptRecord.content} /></div> : null}
            </> : <><div className="visual-handoff-copy"><div><p>Final Script required</p></div><h2>No Script has arrived yet</h2><span>Finish Script and Voiceover first. Visual planning never starts automatically.</span></div><Link href="/studio/scripts" onClick={(e) => studioNavigate('/studio/scripts', e)}>Open Script →</Link></>}
          </section>
          ...
          <section className="visual-control">
            ...
            <div className="visual-model-bar">
              ...
              </> : <div className="visual-no-model"><span>◇</span><div><strong>No AI model connected</strong><small>Add one provider and select a model. Your API key remains in this browser.</small></div><Link href="/studio/settings" onClick={(e) => studioNavigate('/studio/settings', e)}>Open Settings →</Link></div>}
            </div>
            ...
            {!handoffReady ? <div className="visual-prerequisite"><span>!</span><div><strong>Current Voiceover required</strong><p>Prepare a Voiceover from the present Final Script so the clip timing and spoken order cannot drift.</p></div><Link href="/studio/voiceover" onClick={(e) => studioNavigate('/studio/voiceover', e)}>Open Voiceover</Link></div> : null}
          ...
          <footer className="visual-next"><Link href="/studio/voiceover" onClick={(e) => studioNavigate('/studio/voiceover', e)}><span>Previous stage</span><strong>← Voiceover</strong></Link><div><span>Current production idea</span><strong>{selectedIdea?.title ?? 'Nothing selected'}</strong></div><button type="button" disabled={!planCurrent || loading} onClick={continueToAudio}><span>{planCurrent ? 'Complete Visual Plan saved' : 'Build one current plan first'}</span><strong>Audio <i>→</i></strong></button></footer>
        </div>
      </section>
    </main>
  );
